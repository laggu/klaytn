...
) ELSE IF "%NODE_TYPE%"=="PN" (
    set OPTIONS=--nodetype pn --networkid %NETWORK_ID%  --datadir %DATA_DIR%  --port %PORT%  --rpc --rpcapi klay --rpcport %RPC_PORT%  --rpcaddr 0.0.0.0 ^
...
) ELSE IF "%NODE_TYPE%"=="EN" (
    set OPTIONS=--nodetype en --networkid %NETWORK_ID% --datadir %DATA_DIR% --port %PORT% --rpc --rpcapi klay --rpcport %RPC_PORT% --rpcaddr 0.0.0.0 ^
--rpccorsdomain * --rpcvhosts * --ws --wsaddr 0.0.0.0 --wsport %WS_PORT% --wsorigins * --srvtype fasthttp --metrics --prometheus ^
