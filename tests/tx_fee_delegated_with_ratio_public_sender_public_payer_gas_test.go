...
// TestFeeDelegatedWithRatioTransactionGasWithPublicAndPublicPayer checks gas calculations of fee delegated with ratio transaction
...
func TestFeeDelegatedWithRatioTransactionGasWithPublicAndPublicPayer(t *testing.T) {
	if testing.Verbose() {
		enableLog()
	}
	...
	feePayer, err := createHumanReadableAccount("ed580f5bd71a2ee4dae5cb43e331b7d0318596e561e6add7844271ed94156b20", "feePayer")
	assert.Equal(t, nil, err)
	...
	// 1. TxTypeFeeDelegatedValueTransferWithRatio
	{
		...
		err = tx.SignFeePayerWithKeys(signer, feePayer.Keys)
		assert.Equal(t, nil, err)
	...
	// 6. TxTypeFeeDelegatedSmartContractExecutionWithRatio
	{
		...
		data, err := abii.Pack("reward", feePayer.Addr)
		assert.Equal(t, nil, err)
