...
// TestRoleBasedRlpFix tests the following scenario:
// 1. Create an account `colin` with a AccountKeyRoleBasedRlpFix. This should fail.
// 2. Create an account `aidan` with a AccountKeyRoleBased.
// 3. Value transfer (aidan -> reservoir).
// 4. Create an account `colin` with a AccountKeyRoleBasedRlpFix.
// 5. Value transfer (aidan -> reservoir).
// 6. Value transfer (colin -> reservoir).
// 7. Update aidan's key to AccountKeyRoleBased. This should fail.
// 8. Update aidan's key to AccountKeyRoleBasedRlpFix.
func TestRoleBasedRlpFix(t *testing.T) {
	if testing.Verbose() {
		enableLog()
	}
	prof := profile.NewProfiler()
	...
	fork.UpdateHardForkConfig(&fork.HardForkConfig{
		RoleBasedRLPFixBlockNumber: 2,
		GasFormulaFixBlockNumber:   fork.FirstBaobabHardFork,
	})
	...
	colinAddr, err := common.FromHumanReadableAddress("colin")
	keys := genTestKeys(3)
	accKey := accountkey.NewAccountKeyRoleBasedRlpFixWithValues(accountkey.AccountKeyRoleBased{
		accountkey.NewAccountKeyPublicWithValue(&keys[0].PublicKey),
		accountkey.NewAccountKeyPublicWithValue(&keys[1].PublicKey),
		accountkey.NewAccountKeyPublicWithValue(&keys[2].PublicKey),
	})
	...
	// Create an account having a role-based key
	aidanAddr, err := common.FromHumanReadableAddress("aidan")
	aidanKeys := genTestKeys(3)
	aidan := &TestRoleBasedAccountType{
		Addr:       aidanAddr,
		TxKeys:     []*ecdsa.PrivateKey{aidanKeys[0]},
		UpdateKeys: []*ecdsa.PrivateKey{aidanKeys[1]},
		FeeKeys:    []*ecdsa.PrivateKey{aidanKeys[2]},
		Nonce:      uint64(0),
		AccKey: accountkey.NewAccountKeyRoleBasedWithValues(accountkey.AccountKeyRoleBased{
			accountkey.NewAccountKeyPublicWithValue(&aidanKeys[0].PublicKey),
			accountkey.NewAccountKeyPublicWithValue(&aidanKeys[1].PublicKey),
			accountkey.NewAccountKeyPublicWithValue(&aidanKeys[2].PublicKey),
		}),
	}
	...
	// 1. Create an account `colin` with a AccountKeyRoleBasedRlpFix. This should fail.
	{
		...
		assert.Equal(t, nil, err)
		assert.Equal(t, types.ReceiptStatusErrNotSupported, r.Status)
	}
	...
	// 2. Create an account `aidan` with a AccountKeyRoleBased.
	{
		...
		values := map[types.TxValueKeyType]interface{}{
			types.TxValueKeyNonce:         reservoir.Nonce,
			types.TxValueKeyFrom:          reservoir.Addr,
			types.TxValueKeyTo:            aidan.Addr,
			types.TxValueKeyAmount:        amount,
			types.TxValueKeyGasLimit:      gasLimit,
			types.TxValueKeyGasPrice:      gasPrice,
			types.TxValueKeyHumanReadable: true,
			types.TxValueKeyAccountKey:    aidan.AccKey,
		}
		tx, err := types.NewTransactionWithMap(types.TxTypeAccountCreation, values)
		assert.Equal(t, nil, err)
	...
	// 3. Value transfer (aidan -> reservoir).
	{
		...
		amount := new(big.Int).SetUint64(10000)
		values := map[types.TxValueKeyType]interface{}{
			types.TxValueKeyNonce:    aidan.Nonce,
			types.TxValueKeyFrom:     aidan.Addr,
			types.TxValueKeyTo:       reservoir.Addr,
			types.TxValueKeyAmount:   amount,
			types.TxValueKeyGasLimit: gasLimit,
			types.TxValueKeyGasPrice: gasPrice,
		...
		err = tx.SignWithKeys(signer, aidan.TxKeys)
		assert.Equal(t, nil, err)
	...
	// 7. Update aidan's key to AccountKeyRoleBased. This should fail.
	{
		...
		err = tx.SignWithKeys(signer, aidan.UpdateKeys)
		assert.Equal(t, nil, err)

		receipt, _, err := applyTransaction(t, bcdata, tx)
		assert.Equal(t, nil, err)
		assert.Equal(t, types.ReceiptStatusErrDeprecated, receipt.Status)
	}
	...
	fork.UpdateHardForkConfig(&fork.HardForkConfig{
		RoleBasedRLPFixBlockNumber: fork.FirstBaobabHardFork,
		GasFormulaFixBlockNumber:   fork.FirstBaobabHardFork,
	})
