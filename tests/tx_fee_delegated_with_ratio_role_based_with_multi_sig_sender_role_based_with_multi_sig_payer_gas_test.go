...
// TestFeeDelegatedWithRatioTransactionGasWithRoleBasedWithMultiSigAndRoleBasedWithMultiSigPayer checks gas calculations of fee delegated with ratio transaction
...
func TestFeeDelegatedWithRatioTransactionGasWithRoleBasedWithMultiSigAndRoleBasedWithMultiSigPayer(t *testing.T) {
	if testing.Verbose() {
		enableLog()
	}
	...
	roleBasedPayer, err := createRoleBasedAccountWithAccountKeyWeightedMultiSig(
		[]TestCreateMultisigAccountParam{param3, param1, param2}, roleBasedPayerAddr)
	assert.Equal(t, nil, err)
	...
	// Preparing step. Create an AccountKeyRoleBased with AccountKeyWeightedMultiSig.
	{
	...
	// 1. TxTypeFeeDelegatedValueTransferWithRatio
	{
		...
		// TODO-Klaytn-Gas Gas calculation logic has to be changed to calculate only the gas value for the key used in signing.
		sigValidationGasWithTxKeysPayer := params.TxValidationGasDefault + uint64(len(roleBasedPayer.TxKeys))*params.TxValidationGasPerKey
		sigValidationGasWithUpdateKeysPayer := params.TxValidationGasDefault + uint64(len(roleBasedPayer.UpdateKeys))*params.TxValidationGasPerKey
		sigValidationGasWithFeeKeysPayer := params.TxValidationGasDefault + uint64(len(roleBasedPayer.FeeKeys))*params.TxValidationGasPerKey
		gasFeePayer := sigValidationGasWithTxKeysPayer + sigValidationGasWithUpdateKeysPayer + sigValidationGasWithFeeKeysPayer
