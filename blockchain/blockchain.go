...
func (bc *BlockChain) loadLastState() error {
	...
	if currentBlock == nil {
		// Corrupt or empty database, init from scratch
		logger.Warn("Head block missing, resetting chain", "hash", head)
		return bc.Reset()
	}
	// Make sure the state associated with the block is available
	if _, err := state.New(currentBlock.Root(), bc.stateCache); err != nil {
		// Dangling block without a state associated, init from scratch
		logger.Warn("Head state missing, repairing chain", "number", currentBlock.Number(), "hash", currentBlock.Hash())
...
func (bc *BlockChain) FastSyncCommitHead(hash common.Hash) error {
	...
	if _, err := statedb.NewSecureTrie(block.Root(), bc.stateCache.TrieDB(), 0); err != nil {
		return err
	}
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		switch status {
		case CanonStatTy:
			logger.Debug("Inserted new block", "number", block.Number(), "hash", block.Hash(), "uncles", len(block.Uncles()),
