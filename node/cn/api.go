...
func (api *PrivateDebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]common.Address, error) {
	...
	oldTrie, err := statedb.NewSecure(startBlock.Root(), statedb.NewDatabase(api.gxp.chainDb), 0)
	if err != nil {
		return nil, err
	}
	newTrie, err := statedb.NewSecure(endBlock.Root(), statedb.NewDatabase(api.gxp.chainDb), 0)
	if err != nil {
		return nil, err
	}
