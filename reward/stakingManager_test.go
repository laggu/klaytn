...
func TestStakingManager_updateStakingCache(t *testing.T) {
	...
	for i := 0; i < len(testCases); i++ {
		testStakingInfo, error := newEmptyStakingInfo(testCases[i])
		if error != nil {
	...
	for i := 0; i < len(testCases); i++ {
		resultStakingInfo, error := stakingManager.updateStakingCache(testCases[i])
		if error != nil {
			t.Errorf("error has occurred. error : %v", error)
		}
...
func TestStakingManager_getStakingInfoFromStakingCache(t *testing.T) {
	...
	for i := 0; i < len(testData); i++ {
		testStakingInfo, error := newEmptyStakingInfo(testData[i])
		if error != nil {
			t.Errorf("error has occurred. error : %v", error)
			t.FailNow()
		}
