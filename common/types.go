...
type StakingInfo struct {
	...
	CouncilNodeIds       []Address // NodeIds of Council
	CouncilStakingdAddrs []Address // Address of Staking account which holds staking balance
	CouncilRewardAddrs   []Address // Address of Council account which will get block reward
	KIRAddr              Address   // Address of KIR contract
	PoCAddr              Address   // Address of PoC contract

	// Derived from CouncilStakingAddrs
	CouncilStakingAmounts []*big.Int // Staking amounts of Council
}

func (s *StakingInfo) GetIndexByNodeId(nodeId Address) int {
	for i, n := range s.CouncilNodeIds {
		if n == nodeId {
			return i
		}
	}
...
func (s *StakingInfo) GetStakingAmountByNodeId(nodeId Address) *big.Int {
	for i, n := range s.CouncilNodeIds {
		if n == nodeId {
...
func (s StakingCacheKey) getShardIndex(shardMask int) int {
	// TODO-Klaytn-Issue1166 This rule should aligned with reward.StakingUpdateInterval
	return int(s>>4) & shardMask
}
