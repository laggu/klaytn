...
func NewRewardConfigCache(configManager ConfigManager) *rewardConfigCache {
	cache, _ := lru.NewARC(5)
...
func (rewardConfigCache *rewardConfigCache) get(blockNumber uint64) (*rewardConfig, error) {
	epoch := rewardConfigCache.configManager.Epoch()
...
func (rewardConfigCache *rewardConfigCache) newRewardConfig(blockNumber uint64) (*rewardConfig, error) {
	...
	if result, err := rewardConfigCache.configManager.GetItemAtNumberByKey(blockNumber, params.MintingAmount); err == nil {
	...
	if result, err := rewardConfigCache.configManager.GetItemAtNumberByKey(blockNumber, params.Ratio); err == nil {
	...
	if result, err := rewardConfigCache.configManager.GetItemAtNumberByKey(blockNumber, params.UnitPrice); err == nil {
